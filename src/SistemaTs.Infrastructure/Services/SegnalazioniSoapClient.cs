...
public sealed class SegnalazioniSoapClient : SimpleSoapClientBase, ISegnalazioniClient
{
    ...
    public async Task<DettaglioSegnalazioneResponseDto> DettaglioSegnalazioneAsync(
        DettaglioSegnalazioneRequest request,
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(request.Owner.CodiceRegione))
            body += $"<codiceRegione>{request.Owner.CodiceRegione}</codiceRegione>";
        if (!string.IsNullOrEmpty(request.Owner.CodiceAsl))
            body += $"<codiceAsl>{request.Owner.CodiceAsl}</codiceAsl>";
        if (!string.IsNullOrEmpty(request.Owner.CodiceSsa))
            body += $"<codiceSSA>{request.Owner.CodiceSsa}</codiceSSA>";
        body +=
            $"<cfProprietario>{request.Owner.CfProprietario}</cfProprietario></Proprietario>"
            + $"<idDocumentoFiscale>"
        ...
        try
        {
            var xml = await PostSoapAsync(
                _environmentSettings.DettaglioSegnalazioneEndpointUrl,
                "dettagliosegnalazione.p730.sanita.finanze.it",
                request.Credentials,
                envelope,
                ct
            );
    ...
    public async Task<ReportSegnalazioniResponseDto> ReportSegnalazioniAsync(
        ReportSegnalazioniRequest request,
        CancellationToken ct = default
    )
    {
        var pinCode = _cryptoService.EncryptToBase64(request.Credentials.Pincode!);
        ...
        {
            var xml = await PostSoapAsync(
                _environmentSettings.ReportSegnalazioniEndpointUrl,
                "reportsegnalazioni.p730.sanita.finanze.it",
                request.Credentials,
                envelope,
                ct
            );
    ...
    private static DettaglioSegnalazioneResponseDto ParseDettaglioSegnalazioneResponse(string xml)
    {
        ...
        var responseEl = doc.Descendants()
            .FirstOrDefault(e => e.Name.LocalName == "dettaglioSegnalazioneResponse");
        if (responseEl is null)
            return new DettaglioSegnalazioneResponseDto
            {
                Success = false,
                Errors = new[] { "No dettaglioSegnalazioneResponse in response" },
            };
        ...
        if (docEl is not null)
        {
            ...
            if (idEl is not null)
            {
                ...
                {
                    documento.Dispositivo = int.TryParse(
                        GetElementValue(numEl, "dispositivo"),
                        out var d
                    )
                        ? d
                        : 0;
            ...
            {
                foreach (
                    var seg in listaSegEl.Elements().Where(e => e.Name.LocalName == "segnalazione")
                )
            ...
            {
                documento.ListaVociSpesa.Add(
                    new VoceSpesaSegnalazioneDto
                    {
                        TipoSpesa = GetElementValue(voce, "tipoSpesa"),
                        Importo = double.TryParse(GetElementValue(voce, "importo"), out var imp)
                            ? imp
                            : 0,
                        SegnalazioneImporto = GetElementValue(voce, "segnalazioneImporto"),
                        SegnalazioneTipoVoceSpesa = GetElementValue(
                            voce,
                            "segnalazioneTipoVoceSpesa"
                        ),
                    }
                );
            }
            foreach (
                var voce in docEl
                    .Elements()
                    .Where(e => e.Name.LocalName == "listaVociSpesaRimborsate")
            )
            {
                documento.ListaVociSpesaRimborsate.Add(
                    new VoceSpesaSegnalazioneDto
                    {
                        TipoSpesa = GetElementValue(voce, "tipoSpesa"),
                        Importo = double.TryParse(GetElementValue(voce, "importo"), out var imp)
    ...
    private static ReportSegnalazioniResponseDto ParseReportSegnalazioniResponse(string xml)
    {
        var doc = XDocument.Parse(xml);
        var responseEl = doc.Descendants()
            .FirstOrDefault(e => e.Name.LocalName == "reportSegnalazioniResponse");
        if (responseEl is null)
            return new ReportSegnalazioniResponseDto
            {
                Success = false,
                Errors = new[] { "No reportSegnalazioniResponse in response" },
            };
